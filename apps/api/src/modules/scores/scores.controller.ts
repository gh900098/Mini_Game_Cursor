...
export class ScoresController {
  ...
  async submit(
    ...
    if (!req.user.isSuperAdmin && instance.companyId !== companyId) {
      throw new ForbiddenException(
        'You do not have access to this game instance',
      );
    }
  ...
  async getLeaderboardByCompany(
    ...
    const companyId = req.user.companyId;
    const instance = await this.gameInstancesService.findByCompanyAndSlug(
      companySlug,
      gameSlug,
    );
  ...
  async getGameStatus(
    ...
    return this.gameRulesService.getPlayerStatus(
      memberId,
      instance,
      req.user.isImpersonated,
    );
