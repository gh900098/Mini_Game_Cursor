...
export class AdminPrizesController {
  ...
  async getAllPrizes(
    ...
    const query = this.memberPrizeRepo
    ...
    FileInterceptor('file', {
      storage: diskStorage({
        ...
        filename: (req: any, file, cb) => {
          ...
          return cb(
            null,
            `receipt_${Date.now()}_${randomName}${extname(file.originalname)}`,
          );
        },
      }),
        ...
        if (!file.originalname.match(/\.(jpg|jpeg|png|pdf)$/i)) {
          return cb(
            new BadRequestException(
              'Only image files (jpg, jpeg, png) and PDF are allowed for receipts!',
            ),
            false,
          );
        }
