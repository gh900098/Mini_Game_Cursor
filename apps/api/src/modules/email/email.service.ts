...
export class EmailService {
    ...
    async sendVerificationCode(email: string, code: string) {
        ...
        if (provider === 'none') {
            this.logger.warn(`No email provider configured. Verification code for ${email} is: ${code}`);
            return;
        }
    ...
    private async sendSmtp(settings: any, to: string, subject: string, text: string, html: string) {
    ...
    private async sendResend(settings: any, to: string, subject: string, text: string, html: string) {
