...
export class SeedService {
  ...
  private async seedPermissions(): Promise<void> {
    ...
    const specialPermissions = [
      {
        slug: 'members:view_sensitive',
        resource: 'members',
        action: 'view_sensitive',
        name: 'View Sensitive Member Data',
        description: 'View unmasked email and phone number',
      },
    ];
    ...
    for (const resource of deprecatedResources) {
      ...
      const permissionsToDelete = await this.permissionRepository.find({
        where: { resource },
      });
      ...
      if (permissionsToDelete.length > 0) {
        ...
        await this.permissionRepository.query(
          `DELETE FROM role_permissions WHERE "permissionId" IN (${ids})`,
        );
        ...
        if (result.affected && result.affected > 0) {
          this.logger.log(
            `Cleaned up ${result.affected} deprecated permissions for resource: ${resource}`,
          );
        }
      }
    }
  ...
  private async seedRoles(): Promise<void> {
    ...
    for (const roleDef of roleDefinitions) {
      ...
      if (!existing) {
        ...
        this.logger.log(
          `Created role: ${roleDef.name} with ${permissions.length} permissions`,
        );
      } else {
        // FORCE UPDATE ALWAYS
        ...
        this.logger.log(
          `[SEED] Enforced role and permissions: ${roleDef.name} = ${roleDef.level} (${permissions.length} perms)`,
        );
      }
    }
  }
  ...
  private async seedDemoCompany(): Promise<void> {
    ...
    try {
      ...
      await this.companyRepository.query(
        'ALTER TABLE companies ADD COLUMN IF NOT EXISTS "inactiveAt" timestamp',
      );
      ...
      this.logger.error(
        'Failed to manually ensure inactiveAt column: ' + e.message,
      );
    }
    ...
    const adminEmailHash = encryptionServiceInstance
      ? encryptionServiceInstance.hash(adminEmail)
      : undefined;
    ...
    if (!adminUser) {
      ...
      this.logger.log(
        `Created demo admin user: ${adminEmail} with ID: ${adminUser.id}`,
      );
    } else {
      this.logger.log(
        `Found existing demo admin user: ${adminEmail} with ID: ${adminUser.id}`,
      );
    }
    ...
    if (!companyAdminRole) {
      this.logger.error(
        'CRITICAL: company_admin role not found! Cannot assign user to company.',
      );
    ...
    if (!existingAssociation) {
      ...
      try {
        ...
        this.logger.log(
          `Successfully assigned demo user to demo company. association ID: ${saved.id}`,
        );
      } catch (error) {
        this.logger.error(
          `Failed to save UserCompany association: ${error.message}`,
          error.stack,
        );
        throw error;
      }
    ...
    const superEmailHash = encryptionServiceInstance
      ? encryptionServiceInstance.hash(superEmail)
      : undefined;
    let superUser = superEmailHash
      ? await this.userRepository.findOne({
          where: { emailHash: superEmailHash },
        })
  ...
  private async seedThemes(): Promise<void> {
    ...
    if (
      !spinWheelDef ||
      !spinWheelDef.config ||
      !spinWheelDef.config.templatePresets
    ) {
      return;
    }
    ...
    return [
      {
        ...
        thumbnailUrl:
          'https://images.unsplash.com/photo-1596838132731-1820627709c3?w=400&h=250&fit=crop',
        type: 'arcade',
        isActive: true,
        imageSpec: {
          assets: [
            {
              ...
              size: '1080 x 1920 px',
              format: 'WebP',
              note: 'Mobile-first portrait. Under 150KB.',
              mappingKey: 'bgImage',
              category: 'images',
            },
            {
            ...
            },
            {
              ...
              size: '1024 x 1024 px',
              format: 'WebP/PNG',
              note: 'Circular frame around the wheel.',
              mappingKey: 'wheelBorderImage',
              category: 'images',
            },
            {
              name: 'Center Hub',
              size: '256 x 256 px',
              format: 'WebP/PNG',
              note: 'Static center cap.',
              mappingKey: 'centerImage',
              category: 'images',
            },
            {
              name: 'Pointer',
              size: '128 x 256 px',
              format: 'SVG/PNG',
              note: 'Indicator at the top.',
              mappingKey: 'pointerImage',
              category: 'images',
            },
            {
              name: 'Slider Divider',
              size: '64 x 512 px',
              format: 'SVG/PNG',
              note: 'Image for slice separators.',
              mappingKey: 'dividerImage',
              category: 'images',
            },
            {
              name: 'Token Bar',
              size: '600 x 120 px',
              format: 'WebP/PNG',
              note: 'Custom background for balance bar.',
              mappingKey: 'tokenBarImage',
              category: 'images',
            },
            {
              name: 'Spin Button',
              size: '600 x 160 px',
              format: 'WebP/PNG',
              note: 'Bottom interaction button.',
              mappingKey: 'spinBtnImage',
              category: 'images',
            },
            {
              name: 'Win Frame',
              size: '760 x 1000 px',
              format: 'WebP/PNG',
              note: 'Background for win prompt.',
              mappingKey: 'resultWinBackground',
              category: 'images',
            },
            {
              name: 'Lose Frame',
              size: '760 x 1000 px',
              format: 'WebP/PNG',
              note: 'Background for lose prompt.',
              mappingKey: 'resultLoseBackground',
              category: 'images',
            },
            {
              name: 'Win Title',
              size: '600 x 160 px',
              format: 'WebP/PNG',
              note: 'Custom "Congratulations" image.',
              mappingKey: 'resultWinTitleImage',
              category: 'images',
            },
            {
              name: 'Lose Title',
              size: '600 x 160 px',
              format: 'WebP/PNG',
              note: 'Custom "Try Again" image.',
              mappingKey: 'resultLoseTitleImage',
              category: 'images',
            },
            {
              name: 'Win Button',
              size: '560 x 140 px',
              format: 'WebP/PNG',
              note: 'Custom "Collect" button image.',
              mappingKey: 'resultWinButtonImage',
              category: 'images',
            },
            {
              name: 'Lose Button',
              size: '560 x 140 px',
              format: 'WebP/PNG',
              note: 'Custom "Try Again" button image.',
              mappingKey: 'resultLoseButtonImage',
              category: 'images',
            },
            {
              name: 'Jackpot Background',
              size: '760 x 1000 px',
              format: 'WebP/PNG',
              note: 'Background for jackpot prompt.',
              mappingKey: 'jackpotResultBackground',
              category: 'images',
            },
            {
              name: 'Jackpot Title',
              size: '600 x 160 px',
              format: 'WebP/PNG',
              note: 'Custom "JACKPOT" image.',
              mappingKey: 'jackpotResultTitleImage',
              category: 'images',
            },
            {
              name: 'Jackpot Button',
              size: '560 x 140 px',
              format: 'WebP/PNG',
              note: 'Custom jackpot button.',
              mappingKey: 'jackpotResultButtonImage',
              category: 'images',
            },
            {
              name: 'Slice Background',
              size: '256 x 256 px',
              format: 'WebP/PNG',
              note: 'Shared background for slices.',
              mappingKey: 'prizeList.0.icon',
              category: 'images',
            },
            {
              name: 'Prize Icons',
        ...
        config: {
          ...
          templatePresets: {
            ...
            'Zen Oasis': {
              thumbnailUrl:
                'https://images.unsplash.com/photo-1518005020250-675cf8943f01?w=400&h=250&fit=crop',
            ...
            'Candy Kingdom': {
              thumbnailUrl:
                'https://images.unsplash.com/photo-1533910534207-90f31029a78e?w=400&h=250&fit=crop',
        ...
        configSchema: [
          {
            ...
            items: [
              ...
              },
              {
                key: 'prizeList',
                type: 'prize-list',
                label: 'page.manage.game.prizes.prizeSegments',
              },
            ],
          },
          {
            ...
            items: [
              {
                key: 'dailyLimit',
                type: 'number',
                label: 'page.manage.game.rules.dailyLimit',
                default: 3,
                span: 12,
              },
              {
                key: 'cooldown',
                type: 'number',
                label: 'page.manage.game.rules.cooldown',
                default: 60,
                span: 12,
              },
              {
                key: 'minLevel',
                type: 'number',
                label: 'page.manage.game.rules.minLevel',
                default: 0,
                span: 6,
              },
              {
                ...
                items: [
                  {
                    key: 'requireLogin',
                    label: 'page.manage.game.rules.requireLogin',
                  },
                  {
                    key: 'oneTimeOnly',
                    label: 'page.manage.game.rules.oneTimeOnly',
                  },
                ],
              },
              {
                key: 'timeLimitConfig',
                type: 'time-limit',
                label: 'page.manage.game.rules.enableTimeLimit',
                span: 24,
              },
              {
                key: 'dynamicProbConfig',
                type: 'dynamic-prob',
                label: 'page.manage.game.rules.enableDynamicProb',
                span: 24,
              },
              {
                key: 'budgetConfig',
                type: 'budget-control',
                label: 'page.manage.game.rules.enableBudget',
                span: 24,
              },
              {
                key: 'vipTiers',
                type: 'vip-grid',
                label: 'page.manage.game.rules.vipTiers',
                span: 24,
              },
            ],
          },
          {
            ...
            items: [
              ...
              },
              {
                key: 'gameFont',
                type: 'file',
                label: 'page.manage.game.visuals.customFontFile',
                span: 12,
                condition: { key: 'fontPreset', value: 'custom' },
              },
              {
                ...
                items: [
                  ...
                  },
                  {
                    key: 'bgColor',
                    type: 'color',
                    label: 'page.manage.game.visuals.backgroundColor',
                    default: '#1a1a1a',
                    span: 24,
                    condition: { key: 'bgType', value: 'color' },
                  },
                  {
                    key: 'bgImage',
                    type: 'image',
                    label: 'page.manage.game.visuals.backgroundImage',
                    span: 24,
              ...
              // ═══════════════════════════════════════════════════════════
              {
                ...
                items: [
                  {
                    key: 'wheelBorderImage',
                    type: 'image',
                    label: 'page.manage.game.visuals.wheelBorderImage',
                    span: 24,
                  },
                  {
                    key: 'wheelBorderSize',
                    type: 'slider',
                    label: 'page.manage.game.visuals.wheelBorderSize',
                    min: 100,
                    max: 150,
                    step: 1,
                    suffix: '%',
                  ...
                  {
                    key: 'wheelBorderOpacity',
                    type: 'slider',
                    label: 'page.manage.game.visuals.bgOpacity',
                    min: 0,
                    max: 100,
                    step: 5,
                    suffix: '%',
                  ...
                  },
                  {
                    key: 'wheelBorderTop',
                    type: 'slider',
                    label: 'page.manage.game.visuals.verticalOffset',
                    min: -50,
                    max: 50,
                    step: 1,
                    suffix: 'px',
                  ...
                  {
                    key: 'wheelBorderRotation',
                    type: 'slider',
                    label: 'page.manage.game.visuals.rotation',
                    min: 0,
                    max: 360,
                    step: 5,
                    suffix: 'deg',
                  ...
                  },
                  {
                    ...
                    options: [
                      {
                        label: 'page.manage.game.visuals.behindPrizes',
                        value: 'behind',
                      },
                      {
                        label: 'page.manage.game.visuals.inFrontOfPrizes',
                        value: 'front',
                      },
                    ],
                  },
              ...
              },
              {
                ...
                items: [
                  ...
                  },
                  {
                    ...
                    span: 12,
                    condition: { key: 'dividerType', value: 'line' },
                  },
                  {
                    key: 'dividerStroke',
                    type: 'slider',
                  ...
                  },
                  {
                    ...
                    label: 'Texture',
                    span: 24,
                    condition: { key: 'dividerType', value: 'image' },
                  },
                  {
                    key: 'dividerWidth',
                    type: 'slider',
                    label: 'page.manage.game.visuals.width',
                    min: 5,
                    max: 50,
                    step: 1,
                    suffix: 'px',
                ...
                items: [
                  {
                    ...
                    options: [
                      ...
                      {
                        label: 'page.manage.game.visuals.customImage',
                        value: 'image',
                      },
                    ],
                    default: 'emoji',
                  ...
                  },
                  {
                    ...
                    label: 'Hub Image',
                    span: 24,
                    condition: { key: 'centerType', value: 'image' },
                  },
                  {
                    key: 'centerSize',
                    type: 'slider',
                    label: 'page.manage.game.visuals.hubSize',
                    min: 40,
                    max: 120,
                    step: 5,
                    suffix: 'px',
                  ...
                  },
                  {
                    key: 'centerBorder',
                    type: 'switch',
                    label: 'page.manage.game.visuals.border',
                    default: true,
                    span: 7,
                  },
                  {
                    key: 'centerShadow',
                    type: 'switch',
                    label: 'page.manage.game.visuals.shadow',
                    default: true,
                    span: 7,
                  },
                ],
                ...
                items: [
                  {
                    key: 'pointerImage',
                    type: 'image',
                    label: 'page.manage.game.visuals.pointerImage',
                    span: 24,
                  },
                  {
                  ...
                  },
                  {
                    key: 'pointerTop',
                    type: 'slider',
                    label: 'page.manage.game.visuals.topPos',
                    min: -100,
                    max: 100,
                    step: 5,
                    suffix: 'px',
                ...
                items: [
                  {
                    key: 'titleImage',
                    type: 'image',
                    label: 'page.manage.game.visuals.titleImage',
                    span: 24,
                  },
                  {
                    key: 'logoWidth',
                    type: 'slider',
                    label: 'page.manage.game.visuals.logoWidth',
                    min: 20,
                    max: 100,
                    step: 5,
                    suffix: '%',
                  ...
                  },
                  {
                    key: 'logoTopMargin',
                    type: 'slider',
                    label: 'page.manage.game.visuals.topMargin',
                    min: 0,
                    max: 60,
                    step: 2,
                    suffix: 'px',
                  ...
                  },
                  {
                    key: 'logoDropShadow',
                    type: 'switch',
                    label: 'page.manage.game.visuals.enableDropShadow',
                    default: true,
                    span: 12,
                  },
                ],
              },
              {
                ...
                items: [
                  ...
                  {
                    key: 'tokenBarTextColor',
                    type: 'color',
                    label: 'page.manage.game.visuals.textColor',
                    default: '#ffffff',
                    span: 12,
                  },
                  {
                ...
                items: [
                  {
                    key: 'spinBtnText',
                    type: 'text',
                    label: 'page.manage.game.visuals.buttonText',
                    default: 'SPIN NOW',
                    span: 12,
                  },
                  {
                    key: 'spinBtnSubtext',
                    type: 'text',
                    label: 'page.manage.game.visuals.subtext',
                    default: '1 PLAY = 10 TOKEN',
                    span: 12,
                  },
                  {
                    key: 'spinBtnTextColor',
                    type: 'color',
                    label: 'page.manage.game.visuals.textColor',
                    default: '#451a03',
                    span: 8,
                  },
                  {
                    key: 'spinBtnColor',
                    type: 'color',
                    label: 'page.manage.game.visuals.buttonColor',
                    default: '#f59e0b',
                    span: 8,
                  },
                  {
                  ...
                  },
                  {
                    key: 'spinBtnHeight',
                    type: 'slider',
                    label: 'page.manage.game.visuals.height',
                    min: 50,
                    max: 120,
                    step: 5,
                    suffix: 'px',
...

              {
                ...
                items: [
                  // Win Outcome Group
                  {
                    key: 'win_group_title',
                    type: 'divider',
                    label: 'page.manage.game.visuals.winOutcome',
                    span: 24,
                  },
                  {
                    key: 'resultWinBackground',
                    type: 'image',
                    label: 'page.manage.game.visuals.winBackground',
                    span: 12,
                  },
                  {
                    key: 'resultWinTitleImage',
                    type: 'image',
                    label: 'page.manage.game.visuals.winTitleImage',
                    span: 12,
                  },
                  {
                    key: 'winTitle',
                    type: 'text',
                    label: 'page.manage.game.effects.winTitle',
                    default: '🎊 WINNER! 🎊',
                    span: 12,
                  },
                  {
                    key: 'winSubtitle',
                    type: 'text',
                    label: 'page.manage.game.effects.winSubtitle',
                    default: 'Congratulations!',
                    span: 12,
                  },
                  {
                    key: 'resultWinButtonImage',
                    type: 'image',
                    label: 'page.manage.game.visuals.winButtonImage',
                    span: 24,
                  },

                  // Lose Outcome Group
                  {
                    key: 'lose_group_title',
                    type: 'divider',
                    label: 'page.manage.game.visuals.loseOutcome',
                    span: 24,
                  },
                  {
                    key: 'resultLoseBackground',
                    type: 'image',
                    label: 'page.manage.game.visuals.loseBackground',
                    span: 12,
                  },
                  {
                    key: 'resultLoseTitleImage',
                    type: 'image',
                    label: 'page.manage.game.visuals.loseTitleImage',
                    span: 12,
                  },
                  {
                    key: 'loseTitle',
                    type: 'text',
                    label: 'page.manage.game.effects.loseTitle',
                  ...
                  },
                  {
                    key: 'loseSubtitle',
                    type: 'text',
                    label: 'page.manage.game.effects.loseSubtitle',
                    default: 'Try again?',
                    span: 12,
                  },
                  {
                    key: 'resultLoseButtonImage',
                    type: 'image',
                    label: 'page.manage.game.visuals.loseButtonImage',
                    span: 24,
                  },

                  // Jackpot Outcome Group
                  {
                    key: 'jackpot_group_title',
                    type: 'divider',
                    label: 'page.manage.game.visuals.jackpotOutcome',
                    span: 24,
                  },
                  {
                    key: 'jackpotResultBackground',
                    type: 'image',
                    label: 'page.manage.game.visuals.jackpotBackground',
                    span: 12,
                  },
                  {
                    key: 'jackpotResultTitleImage',
                    type: 'image',
                    label: 'page.manage.game.visuals.jackpotTitleImage',
                    span: 12,
                  },
                  {
                    key: 'jackpotResultTitle',
                    type: 'text',
                    label: 'page.manage.game.visuals.jackpotTitle',
                    default: '🎉 JACKPOT!!! 🎉',
                    span: 12,
                  },
                  {
                    key: 'jackpotResultSubtitle',
                    type: 'text',
                    label: 'page.manage.game.visuals.jackpotSubtitle',
                    default: 'INCREDIBLE WIN!',
                    span: 12,
                  },
                  {
                    key: 'jackpotResultButtonImage',
                    type: 'image',
                    label: 'page.manage.game.visuals.jackpotButtonImage',
                    span: 24,
                  },
                ],
              },
                ...
                items: [
                  {
                    key: 'bgmUrl',
                    type: 'file',
                    label: 'page.manage.game.effects.bgmUrl',
                    span: 24,
                  },
                  {
                    key: 'bgmVolume',
                    type: 'slider',
                    label: 'page.manage.game.effects.bgmVolume',
                    min: 0,
                    max: 100,
                    step: 5,
                    suffix: '%',
                    default: 40,
                    span: 12,
                    condition: { key: 'bgmUrl', value: '', operator: 'neq' },
                  },
                  {
                    key: 'bgmLoop',
                    type: 'switch',
                    label: 'page.manage.game.effects.bgmLoop',
                    default: true,
                    span: 12,
                    condition: { key: 'bgmUrl', value: '', operator: 'neq' },
                  },
                ...
                items: [
                  {
                    key: 'winSound',
                    type: 'file',
                    label: 'page.manage.game.effects.winSound',
                    span: 24,
                  },
                  {
                    key: 'loseSound',
                    type: 'file',
                    label: 'page.manage.game.effects.loseSound',
                    span: 24,
                  },
                  {
                    key: 'jackpotSound',
                    type: 'file',
                    label: 'page.manage.game.effects.jackpotSoundFile',
                    span: 24,
                  },
                  {
                    key: 'tickSoundEnabled',
                    type: 'switch',
                    label: 'page.manage.game.effects.tickSound',
                    default: true,
                    span: 12,
                  },
                  {
                    key: 'tickVolume',
                    type: 'slider',
                    label: 'page.manage.game.effects.tickVolume',
                    min: 0,
                    max: 100,
                    step: 5,
                    suffix: '%',
                ...
                items: [
                  {
                    key: 'showSoundButton',
                    type: 'switch',
                    label: 'page.manage.game.effects.showSoundButton',
                    default: true,
                    span: 12,
                  },
                  {
                    key: 'soundButtonOpacity',
                    type: 'slider',
                    label: 'page.manage.game.effects.soundButtonOpacity',
                    min: 0,
                    max: 100,
                    step: 5,
                    suffix: '%',
          ...
          },
          {
            ...
            items: [
              // ⚙️ ANIMATION & INTERACTION
              {
                key: 'spinDuration',
                type: 'slider',
                label: 'page.manage.game.effects.spinDuration',
                min: 1,
                max: 10,
                step: 0.5,
                suffix: 's',
              ...
              },
              {
                key: 'spinTurns',
                type: 'slider',
                label: 'page.manage.game.effects.spinTurns',
                min: 1,
                max: 20,
                step: 1,
                default: 5,
...

              {
                ...
                items: [
                  {
                    key: 'enableBGM',
                    label: 'page.manage.game.effects.enableBGM',
                    default: true,
                  },
                  {
                    key: 'enableLedRing',
                    label: 'page.manage.game.effects.enableLedRing',
                    default: true,
                  },
                  {
                    key: 'enableConfetti',
                    label: 'page.manage.game.effects.enableConfetti',
                    default: true,
                  },
                  {
                    key: 'enableStartScreen',
                    label: 'page.manage.game.effects.enableStartScreen',
                    default: true,
                  },
                  {
                    key: 'enableHexagons',
                    label: 'page.manage.game.effects.enableHexagons',
                    default: true,
                  },
                  {
                    key: 'enableFloatingParticles',
                    label: 'page.manage.game.effects.enableFloatingParticles',
                    default: true,
                  },
                  {
                    key: 'enableGridFloor',
                    label: 'page.manage.game.effects.enableGridFloor',
                    default: true,
                  },
                  {
                    key: 'swipeToSpin',
                    label: 'page.manage.game.effects.swipeToSpin',
                    default: true,
                  },
                ],
              },
                ...
                items: [
                  {
                    key: 'ledCount',
                    type: 'slider',
                    label: 'page.manage.game.effects.ledCount',
                    min: 16,
                    max: 64,
                    step: 4,
                    default: 32,
                  ...
                  },
                  {
                    key: 'ledSpeed',
                    type: 'slider',
                    label: 'page.manage.game.effects.ledSpeed',
                    min: 20,
                    max: 200,
                    step: 10,
                    suffix: 'ms',
                  ...
                  },
                  {
                    key: 'ledColor1',
                    type: 'color',
                    label: 'page.manage.game.effects.ledColor1',
                    default: '#00f5ff',
                    span: 8,
                  },
                  {
                    key: 'ledColor2',
                    type: 'color',
                    label: 'page.manage.game.effects.ledColor2',
                    default: '#ff00ff',
                    span: 8,
                  },
                  {
                    key: 'ledColor3',
                    type: 'color',
                    label: 'page.manage.game.effects.ledColor3',
                    default: '#ffd700',
                    span: 8,
                  },
                ],
                ...
                items: [
                  {
                    ...
                    type: 'slider',
                    label: 'page.manage.game.effects.confettiParticles',
                    min: 50,
                    max: 500,
                    step: 50,
                    default: 200,
                  ...
                  },
                  {
                    key: 'confettiSpread',
                    type: 'slider',
                    label: 'page.manage.game.effects.confettiSpread',
                    min: 30,
                    max: 180,
                    step: 10,
                    suffix: '°',
                  ...
                  },
                  {
                    key: 'confettiColors',
                    type: 'color-list',
                    label: 'page.manage.game.effects.confettiColors',
                    default: '#ff0000,#00ff00,#0000ff,#ffff00,#ff00ff',
                    span: 24,
                  },
                  {
                    key: 'confettiShapeType',
                    type: 'radio',
                    label: 'page.manage.game.effects.confettiShapeType',
                    options: ['default', 'emoji'],
                    default: 'default',
                    span: 24,
                  },
                  {
                    key: 'confettiEmojis',
                    type: 'emoji-list',
                    label: 'page.manage.game.effects.confettiEmojis',
                    default: '🎉,⭐,❤️',
                    span: 24,
                    condition: { key: 'confettiShapeType', value: 'emoji' },
                  },
                ],
              },
              {
                ...
                items: [
                  {
                    key: 'neonCyan',
                    type: 'color',
                    label: 'page.manage.game.effects.neonCyan',
                    default: '#00f5ff',
                    span: 8,
                  },
                  {
                    key: 'neonPink',
                    type: 'color',
                    label: 'page.manage.game.effects.neonPink',
                    default: '#ff00ff',
                    span: 8,
                  },
                  {
                    key: 'neonPurple',
                    type: 'color',
                    label: 'page.manage.game.effects.neonPurple',
                    default: '#9d00ff',
                    span: 8,
                  },
                  {
                    key: 'neonGold',
                    type: 'color',
                    label: 'page.manage.game.effects.neonGold',
                    default: '#ffd700',
                    span: 8,
                  },
                  {
                    key: 'neonGreen',
                    type: 'color',
                    label: 'page.manage.game.effects.neonGreen',
                    default: '#00ff88',
                    span: 8,
                  },
                  {
                    key: 'darkBg',
                    type: 'color',
                    label: 'page.manage.game.effects.darkBg',
                    default: '#0a0a12',
                    span: 8,
                  },
                ],
            ...
            items: [
              {
                key: 'iframeCode',
                type: 'embed-code',
                label: 'page.manage.game.iframeCode',
              },
            ],
          },
        ],
      },
      {
        ...
        thumbnailUrl:
          'https://images.unsplash.com/photo-1518349619113-03114f06ac3a?w=400&h=250&fit=crop',
        type: 'instant',
        isActive: true,
        config: {
          prizeList: [
        ...
        configSchema: [
          {
            name: 'prizes',
            label: 'page.manage.game.common.prizes',
            items: [
              {
                key: 'prizeList',
                type: 'prize-list',
                label: 'page.manage.game.common.prizeList',
              },
            ],
          },
          {
            name: 'settings',
            label: 'page.manage.game.common.settings',
            items: [
              {
                key: 'scratchPercent',
                type: 'slider',
                label: 'page.manage.game.common.scratchPercent',
                min: 30,
                max: 90,
                default: 60,
                suffix: '%',
              },
              {
                key: 'cardColor',
                type: 'color',
                label: 'page.manage.game.common.scratchLayer',
                default: '#c0c0c0',
              },
            ],
          },
      ...
      },
      {
        ...
        configSchema: [
          {
            name: 'rewards',
            label: 'page.manage.game.common.rewards',
            items: [
              {
                key: 'dailyRewards',
                type: 'list',
                label: 'page.manage.game.common.dailyRewards',
              },
              {
                key: 'resetOnMiss',
                type: 'switch',
                label: 'page.manage.game.common.resetOnMiss',
                default: true,
              },
            ],
          },
          {
            name: 'milestones',
            label: 'page.manage.game.common.milestones',
            items: [
              {
                key: 'streakBonus',
                type: 'vip-grid',
                label: 'page.manage.game.common.streakBonus',
              },
            ],
          },
        ],
      },
      {
        ...
        configSchema: [
          {
            name: 'symbols',
            label: 'page.manage.game.common.symbols',
            items: [
              {
                key: 'symbols',
                type: 'list',
                label: 'page.manage.game.common.reelSymbols',
              },
              {
                key: 'payouts',
                type: 'vip-grid',
                label: 'page.manage.game.common.payoutTable',
              },
            ],
          },
          {
            name: 'settings',
            label: 'page.manage.game.common.gameSettings',
            items: [
              {
                key: 'betAmount',
                type: 'number',
                label: 'page.manage.game.common.betAmount',
                default: 10,
              },
              {
                key: 'jackpotAmount',
                type: 'number',
                label: 'page.manage.game.common.jackpotAmount',
                default: 10000,
              },
            ],
          },
      ...
      },
      {
        ...
        configSchema: [
          {
            name: 'board',
            label: 'page.manage.game.common.board',
            items: [
              {
                key: 'rows',
                type: 'slider',
                label: 'page.manage.game.common.numberOfRows',
                min: 8,
                max: 16,
                default: 12,
              },
          ...
          {
            name: 'multipliers',
            label: 'page.manage.game.common.multipliers',
            items: [
              {
                key: 'riskLevels',
                type: 'vip-grid',
                label: 'page.manage.game.common.riskMultipliers',
              },
            ],
          },
        ],
      },
      {
        ...
        configSchema: [
          {
            name: 'gameplay',
            label: 'page.manage.game.common.gameplay',
            items: [
              {
                key: 'gridSize',
                type: 'select',
                label: 'page.manage.game.common.gridSize',
                options: ['3', '4', '5'],
                default: '4',
              },
              {
                key: 'timeLimit',
                type: 'slider',
                label: 'page.manage.game.common.timeLimit',
                min: 30,
                max: 120,
                default: 60,
                suffix: 's',
          ...
          {
            name: 'scoring',
            label: 'page.manage.game.common.scoring',
            items: [
              {
                key: 'baseScore',
                type: 'number',
                label: 'page.manage.game.common.baseScore',
                default: 100,
              },
              {
                key: 'timeBonus',
                type: 'number',
                label: 'page.manage.game.common.timeBonus',
                default: 5,
              },
            ],
          },
      ...
      },
      {
        ...
        thumbnailUrl:
          'https://images.unsplash.com/photo-1522069213448-443a614da9b6?w=400&h=250&fit=crop',
        type: 'casino',
        isActive: true,
        config: {
          betOptions: [
        ...
        configSchema: [
          {
            name: 'bets',
            label: 'page.manage.game.common.bets',
            items: [
              {
                key: 'betOptions',
                type: 'vip-grid',
                label: 'page.manage.game.common.bettingOptions',
              },
              {
                key: 'betAmount',
                type: 'number',
                label: 'page.manage.game.common.defaultBet',
                default: 50,
              },
            ],
          },
      ...
      },
      {
        ...
        thumbnailUrl:
          'https://images.unsplash.com/photo-1553991562-9f24b119ff51?w=400&h=250&fit=crop',
        type: 'instant',
        isActive: true,
        config: {
          boxCount: 9,
        ...
        configSchema: [
          {
            name: 'boxes',
            label: 'page.manage.game.common.boxes',
            items: [
              {
                key: 'boxCount',
                type: 'number',
                label: 'page.manage.game.common.numberOfBoxes',
                default: 9,
              },
              {
                key: 'picksAllowed',
                type: 'number',
                label: 'page.manage.game.common.picksAllowed',
                default: 3,
              },
            ],
          },
            ...
            items: [
              {
                key: 'prizeList',
                type: 'prize-list',
                label: 'page.manage.game.common.prizeConfiguration',
              },
            ],
          },
        ],
      },
      {
        ...
        thumbnailUrl:
          'https://images.unsplash.com/photo-1606326608606-aa0b62935f2b?w=400&h=250&fit=crop',
        type: 'trivia',
        isActive: true,
        config: {
          categories: [
        ...
        configSchema: [
          {
            name: 'categories',
            label: 'page.manage.game.common.categories',
            items: [
              {
                key: 'categories',
                type: 'list',
                label: 'page.manage.game.common.quizCategories',
              },
            ],
          },
          {
            ...
            items: [
              {
                key: 'timePerQuestion',
                type: 'slider',
                label: 'page.manage.game.common.timePerQuestion',
                min: 5,
                max: 30,
                default: 10,
                suffix: 's',
              },
              {
                key: 'basePoints',
                type: 'number',
                label: 'page.manage.game.common.basePoints',
                default: 100,
              },
              {
                key: 'streakBonus',
  ...
  private async seedGames(): Promise<void> {
    ...
    for (const gameData of gameDefs) {
      ...
      if (!existing) {
        const game = this.gameRepository.create(gameData) as any;
        ...
        existing.configSchema = JSON.parse(
          JSON.stringify(gameData.configSchema),
        );
  ...
  private async seedInstances(): Promise<void> {
    ...
    const spinTemplate = await this.gameRepository.findOne({
    ...
    if (company && spinTemplate) {
      ...
      for (const instanceData of instances) {
        const existing = await this.instanceRepository.findOne({
          where: { slug: instanceData.slug },
        });
        if (!existing) {
          await this.instanceRepository.save(
            this.instanceRepository.create(instanceData),
          );
