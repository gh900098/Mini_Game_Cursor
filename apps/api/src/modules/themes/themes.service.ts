...
export class ThemesService {
  ...
  async findAll(
    ...
    if (gameTemplateSlug) {
      query.andWhere('theme.gameTemplateSlug = :gameTemplateSlug', {
        gameTemplateSlug,
      });
    }
    ...
    if (companyId) {
      ...
      query.andWhere(
        '(theme.companyId IS NULL OR theme.companyId = :companyId)',
        { companyId },
      );
    } else {
      // Only global themes
  ...
  async update(id: string, updateThemeDto: UpdateThemeDto): Promise<Theme> {
    ...
    if (updateThemeDto.slug && updateThemeDto.slug !== theme.slug) {
      ...
      if (existingTheme) {
        throw new ConflictException(
          `Theme with slug '${updateThemeDto.slug}' already exists`,
        );
      }
    }
