...
export class AuthController {
  ...
  async isVerificationRequired() {
    const required = await this.settingsService.getSetting(
  ...
  @ApiResponse({
    status: 200,
    description: 'Returns JWT access token with company context',
  })
  ...
  async getProfile(@Request() req: any) {
    ...
    const currentCompany = user.userCompanies?.find(
      (uc) => uc.companyId === req.user.currentCompanyId,
    )?.company;
  ...
  @ApiResponse({
    status: 200,
    description: 'Returns new JWT with updated company context',
  })
  @ApiResponse({
    status: 400,
