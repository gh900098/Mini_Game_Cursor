...
export class AuthService {
  ...
  async validateUser(email: string, pass: string): Promise<any> {
    ...
    const verificationRequired = await this.settingsService.getSetting(
  ...
  async login(user: any) {
    ...
    const primaryCompany =
      userCompanies.find((uc: any) => uc.isPrimary) || userCompanies[0];
    ...
    const payload = {
      ...
      permissions:
        primaryCompany?.role?.permissions?.map((p: any) => p.slug) || [],
  ...
  async switchCompany(userId: string, companyId: string) {
    ...
    if (companyId === 'ALL') {
      const superAdminUc = user.userCompanies?.find(
        (uc: any) => uc.role?.slug === 'super_admin',
      );

      if (!superAdminUc) {
        throw new ForbiddenException(
          'Only Super Admins can switch to Global view',
        );
      }
      ...
      const payload = {
        ...
        permissions:
          superAdminUc.role?.permissions?.map((p: any) => p.slug) || [],
    ...
    let userCompany = user.userCompanies?.find(
      (uc: any) => uc.companyId === companyId,
    );
    const superAdminUc = user.userCompanies?.find(
      (uc: any) => uc.role?.slug === 'super_admin',
    );
