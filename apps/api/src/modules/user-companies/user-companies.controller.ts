...
export class UserCompaniesController {
    ...
    @ApiResponse({ status: 201, description: 'Company access added successfully' })
    @ApiResponse({ status: 404, description: 'User, company, or role not found' })
    @ApiResponse({ status: 409, description: 'User already has access to this company' })
    ...
    removeCompany(@Param('userId') userId: string, @Param('companyId') companyId: string) {
    ...
    updateRole(
        ...
        return this.userCompaniesService.updateUserCompanyRole(userId, companyId, roleId);
    }
    ...
    setPrimary(@Param('userId') userId: string, @Param('companyId') companyId: string) {
