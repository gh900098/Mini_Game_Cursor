...
export class SyncProcessor extends WorkerHost {
  ...
  async process(job: Job<any, any, string>): Promise<any> {
    ...
    this.logger.log(
      `[v2.1] Processing ${type} sync for company ${companyId} (Source: ${source || 'unknown'})`,
    );
    ...
    try {
      ...
      if (type === 'hourly-sync' || type === 'master_trigger') {
        this.logger.log(
          'Starting hourly full sync orchestration (Master Trigger)...',
        );
      ...
      if (type === 'company-batch' && companyId) {
        ...
        if (
          !company ||
          !company.integration_config?.enabled ||
          !company.integration_config?.provider
        ) {
          this.logger.warn(
            `Specific company sync failed: Company ${companyId} not found or integration disabled/missing provider.`,
          );
          return { success: false, reason: 'Inactive or Missing' };
        }
        const syncType = job.data.syncType || 'member';
        this.logger.log(
          `Starting granular ${syncType} sync for company: ${company.name} (${companyId})`,
        );
      ...
      if (
        ...
        this.logger.warn(
          `Integration disabled or company/provider not found: ${companyId}`,
        );
      ...
      this.logger.error(
        `Sync failed for type ${type} in company ${companyId}: ${error.message}`,
        error.stack,
      );
  ...
  private async handleHourlySyncOrchestration() {
    const { items: companies } = await this.companiesService.findAll({
      limit: 1000,
    });
    const enabledCompanies = companies.filter(
      (c) => c.integration_config?.enabled && c.integration_config?.provider,
    );

    this.logger.log(
      `Starting dynamic sync orchestration for ${enabledCompanies.length} enabled companies...`,
    );
      ...
      const results = await Promise.all(
        batch.map(async (company) => {
          try {
            ...
            const memberResult = await strategy.execute(
              company.id,
              company.integration_config,
              { type: 'member' },
            );
            const depositResult = await strategy.execute(
              company.id,
              company.integration_config,
              { type: 'deposit' },
            );
            ...
            const memberJobs =
              typeof memberResult === 'number' ? memberResult : 0;
            const depositJobs =
              typeof depositResult === 'number' ? depositResult : 0;

            return memberJobs + depositJobs;
          } catch (e) {
            this.logger.error(
              `Error orchestrating sync for company ${company.id}: ${e.message}`,
            );
            return 0;
          }
        }),
    ...
    this.logger.log(
      `Dynamic sync orchestration completed. Total jobs queued across all companies: ${totalQueued}`,
    );
    return {
      success: true,
