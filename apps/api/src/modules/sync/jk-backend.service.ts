...
export class JKBackendService {
  ...
  private async request(
    ...
    try {
      ...
      this.logger.debug(
        `JK API Request: POST ${targetUrl} Data: ${body.toString()}`,
      );
      ...
      if (config?.proxy?.enabled && config.proxy.host && config.proxy.port) {
        ...
        this.logger.log(
          `Using proxy: ${protocol}://${host}:${port} for ${targetUrl}`,
        );
      ...
      this.logger.debug(
        `JK API Response [${targetUrl}]: ${JSON.stringify(response.data)}`,
      );
      ...
      if (error.response) {
        this.logger.error(
          `Request failed [${errorUrl}]: ${error.message} - Status: ${error.response.status} - Data: ${JSON.stringify(error.response.data)}`,
        );
      } else {
  ...
  async getAllTransactions(config: any, pageIndex = 0, filters: any = {}) {
    ...
    const syncDays = Math.min(
      Math.max(Number(config.syncConfigs?.['deposit']?.syncDays || 2), 1),
      30,
    );
