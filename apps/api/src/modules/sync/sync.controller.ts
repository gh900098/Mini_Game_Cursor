...
export class SyncController {
  ...
  async handleWebhook(
    ...
    if (
      ...
      company.integration_config?.ipWhitelistEnabled
    ) {
      const clientIp =
        req.ip ||
        ...
        '';
      const whitelistedIps = (company.integration_config.ipWhitelist || '')
        .split(',')
        .map((ip) => ip.trim());

      if (!this.isIpWhitelisted(clientIp, whitelistedIps)) {
        this.logger.warn(
          `Rejected webhook from non-whitelisted IP: ${clientIp} for company ${companyId}`,
        );
