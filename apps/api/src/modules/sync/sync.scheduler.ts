...
export class SyncScheduler implements OnModuleInit {
  ...
  async refreshScheduler() {
    ...
    const globalCron =
      (await this.settingsService.getSetting('sync_hourly_cron')) ||
      '0 */4 * * *';
    ...
    if (legacyMembers.length > 0) {
      this.logger.warn(
        `Found ${legacyMembers.length} legacy repeat entry(ies) in Redis. Cleaning up...`,
      );
    ...
    for (const company of enabledCompanies) {
      ...
      for (const type of SYNC_TYPES) {
        ...
        const isTypeEnabled = typeConfig
          ? typeConfig.enabled
          : type === 'member'; // Default member enabled if root is enabled

        if (!isTypeEnabled) continue;

        const typeCron =
          typeConfig?.syncCron ||
          company.integration_config?.syncCron ||
          globalCron;

        this.logger.log(
          `Registering ${type} sync for company ${company.id} (${company.name}) [Provider: ${company.integration_config.provider}] with schedule: ${typeCron}`,
        );
    ...
    this.logger.log(
      `Granular scheduler refreshed for ${enabledCompanies.length} companies.`,
    );
  }
}
