...
export class AuditLogService {
  ...
  async findAll(query: any) {
    ...
    if (userName) {
      qb.andWhere('auditLog.userName ILIKE :userName', {
        userName: `%${userName}%`,
      });
    }
    ...
    if (companyId || ownUserId) {
      if (companyId && ownUserId) {
        ...
        qb.andWhere(
          '(auditLog.companyId = :companyId OR auditLog.userId = :ownUserId)',
          { companyId, ownUserId },
        );
      } else if (companyId) {
    ...
    console.log(`[AuditLogService] Found ${total} logs for query:`, {
      pageNum,
      limitNum,
      companyId,
      module,
