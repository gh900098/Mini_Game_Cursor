import { Controller, Get, Query, Param, UseGuards, ParseUUIDPipe, Req, ForbiddenException, Logger } from '@nestjs/common';
    ...
    findAll(@Req() req: any, @Query() query: any) {
        ...
        if (!isSuperAdmin) {
            ...
            if (!query.companyId && !query.ownUserId) {
                this.logger.warn(`User ${user?.userId} attempted to view audit logs without a valid context`);
                throw new ForbiddenException('You must belong to a company to view audit logs');
            }
        }
