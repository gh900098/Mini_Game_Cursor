...
export class GameInstancesService {
   ...
   */
  async enrichConfigWithPrizeMeta(
  ...
  async isBudgetExhausted(instance: GameInstance): Promise<boolean> {
    ...
    if (config.dailyBudget) {
      ...
      if (Number(todayTracking?.totalCost || 0) >= config.dailyBudget)
        return true;
    }
    ...
    if (config.monthlyBudget) {
      ...
      const monthTracking = await this.budgetRepo
        ...
        .getRawOne();
      if (Number(monthTracking?.total || 0) >= config.monthlyBudget)
        return true;
    }

    // 3. Total/Activity Budget
    if (config.totalBudget) {
      const totalTracking = await this.budgetRepo
        .createQueryBuilder('bt')
        .where('bt.instanceId = :instanceId', { instanceId: instance.id })
        .select('SUM(bt.totalCost)', 'total')
        .getRawOne();
  ...
  async findBySlug(slug: string): Promise<GameInstance> {
    ...
    if (!instance) {
      throw new NotFoundException(
        `Game instance with slug "${slug}" not found`,
      );
    }
    return instance;
  }
  ...
  async findByCompanyAndSlug(
    ...
    if (!instance) {
      throw new NotFoundException(
        `Game instance "${gameSlug}" not found for company "${companySlug}"`,
      );
    }
    return instance;
  }
  ...
  async findByCompanyIdAndSlug(
    ...
    if (!instance) {
      throw new NotFoundException(
        `Game instance "${gameSlug}" not found for company ID "${companyId}"`,
      );
    }
    return instance;
  }
  ...
  async remove(id: string): Promise<{ result: string; message: string }> {
    ...
    const recordCount = await this.scoreRepository.count({
      where: { instanceId: id },
    });
    ...
    if (recordCount > 0) {
      ...
      return {
        result: 'soft_delete',
        message: `Instance has ${recordCount} records. Marked as inactive.`,
      };
    } else {
      // Hard delete
  ...
  async checkUsage(
    id: string,
  ): Promise<{ hasRecords: boolean; recordCount: number }> {
  ...
  async validateSlug(
    ...
    const suggestedSlug = await this.ensureUniqueSlug(
      slug,
      companyId,
      excludeId,
    );
