...
export class PrizesService {
  ...
  async resolveType(idOrSlug: string, companyId?: string): Promise<PrizeType> {
    if (this.isUuid(idOrSlug)) {
      const type = await this.prizeTypeRepo.findOne({
        where: { id: idOrSlug },
      });
      if (type) return type;
    }
  ...
  async seedDefaults() {
    const defaultTypes = [
      {
        name: 'Item',
        slug: 'item',
        strategy: PrizeStrategy.MANUAL_FULFILL,
        showValue: false,
        icon: '🎁',
        description: 'Physical items requiring manual fulfillment',
        companyId: null,
        isPoints: false,
      },
      {
        name: 'E-Gift',
        slug: 'egift',
        strategy: PrizeStrategy.VIRTUAL_CODE,
        showValue: true,
        icon: '📧',
        description: 'Digital codes/coupons',
        companyId: null,
        isPoints: false,
      },
      {
        ...
        showValue: true,
        icon: '💰',
        description: 'Cash payout (requires approval)',
        companyId: null,
        isPoints: false,
      },
      {
        name: 'Points',
        slug: 'points',
        strategy: PrizeStrategy.BALANCE_CREDIT,
        showValue: true,
        icon: '⚪',
        description: 'Loyalty points credit',
        companyId: null,
        isPoints: true,
      },
    ];
    ...
    for (const typeData of defaultTypes) {
      const existing = await this.prizeTypeRepo.findOne({
        where: { slug: typeData.slug, companyId: IsNull() },
      });
      if (!existing) {
