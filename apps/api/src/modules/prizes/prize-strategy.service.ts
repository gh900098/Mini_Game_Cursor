...
export class PrizeStrategyService {
  ...
  private async handleBalanceCredit(memberId: string, amount: number) {
    try {
      ...
      return {
        status: 'fulfilled',
        note: `Automatically credited ${amount} points/balance.`,
      };
    } catch (error) {
      this.logger.error(
        `Balance credit failed for member ${memberId}: ${error.message}`,
      );
      return {
        status: 'pending',
        note: `Auto-credit failed: ${error.message}. Manual retry needed.`,
      };
    }
  }

  private async handleExternalHook(
    memberId: string,
    prizeType: PrizeType,
    metadata: any,
  ) {
    ...
    if (!webhookUrl) {
      return {
        status: 'pending',
        note: 'External activation failed: Webhook URL not configured.',
      };
    }

    try {
      // Replace placeholders in config template if exists
      const payload = prizeType.config?.payload || {
        memberId,
      ...
      if (response.ok) {
        return {
          status: 'fulfilled',
          note: 'Activated via external system successfully.',
        };
      } else {
        const errorText = await response.text();
        return {
          status: 'pending',
          note: `External system returned error: ${errorText}`,
        };
      }
    } catch (error) {
      ...
      return {
        status: 'pending',
        note: `Connection to external system failed: ${error.message}`,
      };
    }
  }
}
