...
export class AuditInterceptor implements NestInterceptor {
    ...
    private async logAction(request: any, result: any, duration: number, status: number) {
        try {
            const { method, path, body, params, query, ip, user } = request;
            const userAgent = request.get('user-agent') || '';
            ...
            const startIndex = parts.findIndex((p: string) => p !== 'api' && !/^v\d+$/.test(p));
            const moduleName = startIndex !== -1 ? parts[startIndex] : 'unknown';

            // 0. Check System Settings (Optimization)
            const auditConfig = await this.settingsService.getSetting('AUDIT_LOG_CONFIG');
            if (auditConfig) {
                // If global disable
                if (auditConfig.enabled === false) return;

                // If specific module disable
                if (auditConfig.modules && auditConfig.modules[moduleName] === false) return;
            }

            // 2. Resolve User Context (UserId, UserName, CompanyId)
            let userId = user?.userId;
            let userName = user?.email;
            let companyId = user?.currentCompanyId || body?.companyId || query?.companyId;
            ...
            if (idMap.permissionId) {
                try {
                    const permission = await this.permissionsService.findOne(idMap.permissionId);
            ...
            const isLoginOrSwitch = (path.includes('/auth/login') || path.includes('/auth/switch-company'));
                ...
                try {
                    ...
                    if (decoded) {
                        ...
                        if (newCompanyId) {
                            ...
                            try {
                                const company = await this.companiesService.findOne(newCompanyId);
                                if (company) {
                    ...
                    this.logger.warn(`Failed to decode token in AuditInterceptor: ${e.message}`);
                }
            }
    ...
    private getActionName(method: string, path: string): string {
        const parts = path.split('/').filter(p => !!p && p !== 'api' && !/^v\d+$/.test(p));
        ...
        if (pathStr.includes('/auth/')) {
            ...
            if (pathStr.includes('/send-password-reset-code')) return 'Request Password Reset';
            if (pathStr.includes('/verify-password-reset-code')) return 'Verify Reset Code';
            if (pathStr.includes('/send-registration-code')) return 'Request Registration Code';
            if (pathStr.includes('/verify-email')) return 'Verify Email';
            if (pathStr.includes('/resend-verification')) return 'Resend Verification';
        }
        ...
        if (pathStr.startsWith('/roles')) {
            ...
            if (method === 'PATCH' || method === 'PUT') return 'Update Role & Permissions';
        ...
        if (pathStr.startsWith('/permissions')) {
            ...
            if (method === 'PATCH' || method === 'PUT') return 'Update Permission Definition';
        ...
        if (pathStr.startsWith('/companies')) {
            ...
            if (method === 'PATCH' || method === 'PUT') return 'Update Company Profile';
            if (method === 'DELETE') return 'Remove Company Record';
        }

        // SYSTEM SETTINGS
        if (pathStr.includes('/system-settings')) {
            if (method === 'PATCH' || method === 'PUT') return 'Update System Settings';
        }
        ...
        if (isId) {
            ...
            const commonActions = ['login', 'register', 'upload', 'send', 'reset', 'verify', 'seed', 'sync'];
