...
function filterAuthRouteByRoles(route: ElegantConstRoute, roles: string[], permissions: string[]): ElegantConstRoute[] {
  ...
  if (routePermission) {
    const [resource] = routePermission.split(':');
    hasPermission = permissions.includes(routePermission) || permissions.includes(`${resource}:manage`);
  }

  // Check if user has R_SUPER role
  const isSuper = roles.includes('R_SUPER');

  // The route is allowed if:
  // 1. User is Super Admin (R_SUPER)
  // OR
  // 2. It matches the role (or roles are empty) AND it matches the permission
  const isSelfAllowed = isSuper || ((hasRole || isEmptyRoles) && hasPermission);

  const filterRoute = { ...route } as ElegantConstRoute;

  if (filterRoute.children?.length) {
    const originalChildrenCount = filterRoute.children.length;
    filterRoute.children = filterRoute.children.flatMap(item => filterAuthRouteByRoles(item, roles, permissions));

    // If the route has children, and after filtering SOME children remain, 
    // we should allow this parent even if it doesn't have explicit roles/permissions
    if (filterRoute.children.length > 0) {
      return [filterRoute];
    }

    // If it had children but NO children remain after filtering, hide the parent
    if (originalChildrenCount > 0) {
      return [];
    }
  }

  // If it's a leaf node, return it only if it's allowed
  return isSelfAllowed ? [filterRoute] : [];
}
